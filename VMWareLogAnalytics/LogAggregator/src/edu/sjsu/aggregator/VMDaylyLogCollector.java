...
public class VMDaylyLogCollector implements Runnable{
final static Logger logger = Logger.getLogger(VMLogAggregator.class);
	...
	public VMDaylyLogCollector(DBProperties properties, ConfigProperties configProperties){
	...
	public void run(){
		while(!threadStopper){
			try {
				aggregatorThread.sleep(24*50*60*1000);
	...
	public void getAverage(String ip, String datetime) throws UnknownHostException, ClassNotFoundException, SQLException, ParseException{
		...
		DBCursor cursor = stats.find(query).sort(new BasicDBObject("logdate", -1));
	 	...
	 	DBCollection collection = db.getCollection("bkplogdaily");
		...
		DBCollection aggregateStats = db.getCollection("bkplogdaily");
	...
	private String insertVMQuery(DBObject dbObject, String date) {
		...
		String sql = "INSERT INTO vmstatsdaily (`vmname`,`cpu_usage`,`cpu_used`,`cpu_usagemhz`,`cpu_system`,`mem_shared`,`mem_granted`,`mem_usage`,`mem_consumed`,"

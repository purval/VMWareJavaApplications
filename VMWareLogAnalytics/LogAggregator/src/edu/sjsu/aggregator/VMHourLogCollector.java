...
public class VMHourLogCollector implements Runnable{
final static Logger logger = Logger.getLogger(VMLogAggregator.class);
	...
	public VMHourLogCollector(DBProperties properties, ConfigProperties configProperties){
	...
	public void run(){
		while(!threadStopper){
			try {
				aggregatorThread.sleep(59*60*1000);
	...
	public void getAverage(String ip, String datetime) throws UnknownHostException, ClassNotFoundException, SQLException, ParseException{
		...
		DBCursor cursor = stats.find(query).sort(new BasicDBObject("logdate", -1));
	 	...
	 	DBCollection collection = db.getCollection("bkplogHour");
		...
		DBCollection aggregateStats = db.getCollection("bkplogHour");
	...
	private String insertVMQuery(DBObject dbObject, String date) {
		...
		String sql = "INSERT INTO vmstatshourly (`vmname`,`cpu_usage`,`cpu_used`,`cpu_usagemhz`,`cpu_system`,`mem_shared`,`mem_granted`,`mem_usage`,`mem_consumed`,"
